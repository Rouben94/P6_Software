...
#Get all Clients from Dataset
def getClients(df):
    filter_values = {'RSSI': ['0']}
    row_mask = df.isin(filter_values).any(1)
    clients = df[row_mask].drop_duplicates(subset ='Source Address',keep = 'first', inplace = False) # Only keep the First
    return clients
#Get all Servers from Dataset
def getServers(df):
    filter_values = {'RSSI': ['0']}
    df2 = df.isin(filter_values)
    row_mask = df2.all(0)
    servers = df2[row_mask]
    servers_unique = servers.drop_duplicates(subset ='Destination Address',keep = 'first', inplace = False) # Only keep the First
    return servers_unique
#Get all Servers for a specific Group
def getServersByGroup(df,GrpAddr):
    filter_values = {'Group Address': [GrpAddr]}
    row_mask = getServers(df).isin(filter_values).any(1)
    servers_grp = df[row_mask]
    return servers_grp
#Get all Messages betwenn a specific client and server
def getClientServerMessages(df,DstAddr,GrpAddr):
    filter_values = {'Source Address': [SrcAddr], 'Destination Address': [DstAddr , GrpAddr]}
    row_mask = df.isin(filter_values).all(1)
    df[row_mask]
