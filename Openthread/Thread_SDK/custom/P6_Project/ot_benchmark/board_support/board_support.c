/** @file
 *
 * @defgroup ot_benchmark board_support
 * @{
 * @ingroup ot_benchmark
 * @brief OpenThread benchmark Application board_support file.
 *
 * @details This file sets the board settings.
 *
 */
...
uint32_t bsp_init(uint32_t type, bsp_event_callback_t callback)
{
    ...
    if (type & BSP_INIT_BUTTONS)
    {
        ...
        for (num = 0; ((num < BUTTONS_NUMBER) && (err_code == NRF_SUCCESS)); num++)
        {
            ...
            err_code = bsp_event_to_button_action_assign(num, BSP_BUTTON_ACTION_LONG_PUSH, BSP_EVENT_KEY_0_LONG);
        }
