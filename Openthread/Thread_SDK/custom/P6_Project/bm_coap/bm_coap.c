...
bm_master_message start_message;
...
void bm_coap_results_send(bm_message_info *message_info)
{
    ...
    do
    {
        ...
        error = otMessageAppend(p_request, message_info, sizeof(bm_message_info));
        if (error != OT_ERROR_NONE)
        {
            break;
        }
                             ...
                             const otMessageInfo  * p_message_info)
{
    ...
    do
    {
        ...
        if (otMessageRead(p_message, otMessageGetOffset(p_message), &start_message, sizeof(bm_master_message)) != 31)
        {
            NRF_LOG_INFO("benchmark request handler - missing message")
        }

        bm_start_param_set(&start_message);
        bsp_board_led_blink(BSP_BOARD_LED_2);
        bm_master_address = start_message.master_address;

        NRF_LOG_INFO("Slave: Got broad cast");
        if (m_config.coap_client_enabled)
        {
            bm_sm_new_state_set(BM_STATE_1_CLIENT);
        }
        if (m_config.coap_server_enabled)
        {
            bm_sm_new_state_set(BM_STATE_1_SERVER);
        }
    } while(false);
}
...
void bm_coap_multicast_start_send(bm_master_message * message)
{
    ...
    do
    {
        ...
        error = otMessageAppend(p_request, message, sizeof(bm_master_message));
        if (error != OT_ERROR_NONE)
        {
            break;
        }
...
void bm_coap_probe_message_send(uint16_t size)
{
    ...
    do
    {
        ...
        otRandomNonCryptoFillBuffer(bm_probe, size);
